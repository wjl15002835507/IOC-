...
export const createXlsxTemplateBytes = (fields: string[], sample: Record<string, unknown>, validations: TemplateValidation[], dataRows: Record<string, unknown>[] = [], editableFields: string[] = []) => {
  ...
  const rows = [
    ...
    ...rowData.map((row, rowIndex) => `<row r="${rowIndex + 2}">${fields.map((field, index) => inlineCell(`${columnName(index)}${rowIndex + 2}`, row[field] ?? '', editable.has(field) ? 2 : 0)).join('')}</row>`),
  ].join('');
  ...
  const files = [
    ...
    { name:'xl/styles.xml', content:'<?xml version="1.0" encoding="UTF-8" standalone="yes"?><styleSheet xmlns="http://schemas.openxmlformats.org/spreadsheetml/2006/main"><fonts count="2"><font><sz val="11"/><name val="Microsoft YaHei"/></font><font><b/><color rgb="FFFFFFFF"/><sz val="11"/><name val="Microsoft YaHei"/></font></fonts><fills count="3"><fill><patternFill patternType="none"/></fill><fill><patternFill patternType="gray125"/></fill><fill><patternFill patternType="solid"><fgColor rgb="FF0C9B78"/><bgColor indexed="64"/></patternFill></fill></fills><borders count="1"><border><left/><right/><top/><bottom/><diagonal/></border></borders><cellStyleXfs count="1"><xf numFmtId="0" fontId="0" fillId="0" borderId="0"/></cellStyleXfs><cellXfs count="3"><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"/><xf numFmtId="0" fontId="1" fillId="2" borderId="0" xfId="0" applyFont="1" applyFill="1"><alignment horizontal="center" vertical="center"/></xf><xf numFmtId="0" fontId="0" fillId="0" borderId="0" xfId="0"><protection locked="0"/></xf></cellXfs><cellStyles count="1"><cellStyle name="Normal" xfId="0" builtinId="0"/></cellStyles></styleSheet>' },
...
export const createXlsxTemplate = (fields: string[], sample: Record<string, unknown>, validations: TemplateValidation[], dataRows: Record<string, unknown>[] = [], editableFields: string[] = []) => {
  const bytes = createXlsxTemplateBytes(fields, sample, validations, dataRows, editableFields);
