...
import { ChevronLeft, ChevronRight, Download, FileSpreadsheet, History, RotateCcw, Search, Upload, X } from 'lucide-react';
...
const importFieldKey: Record<string, string> = { '工厂编码':'factorySet', '工厂描述':'factoryName', '物料编码':'material', '物料描述':'materialDescription', '处理方案类型':'solution', '处理意见':'opinion', '方案制定部门':'solutionDepartment', '留用数量':'retainedQuantity', '留用时间':'retainedDate', '预计消耗完成时间':'expectedDate', '预计打样完成时间':'expectedSampleDate', '责任单位':'responsibleUnit', '静态原因':'staticReason' };

const strategicRows: Row[] = [
  { category:'五金', materialCode:'117000001', materialDescription:'直径1.4钢丝', baselineAverageConsumption:'4,820.16', preparationDays:'30天', currentStock:'174,993.03', averageConsumption:'4,929.38', supplyDays:'35.5', overstockQuantity:'27,111.59' },
  { category:'板材', materialCode:'101014131', materialDescription:'18mm双面星辰链结哑光三胺板', baselineAverageConsumption:'20.40', preparationDays:'45天', currentStock:'824.00', averageConsumption:'18.60', supplyDays:'44.3', overstockQuantity:'173.00' },
  { category:'封边带', materialCode:'104009061', materialDescription:'H214#墨韵细木纹PVC浮雕封边带', baselineAverageConsumption:'16.25', preparationDays:'60天', currentStock:'1,128.00', averageConsumption:'14.70', supplyDays:'76.7', overstockQuantity:'245.00' },
];
...
const newStaticStaticColumns: Column[] = [
  {key:'material',label:'物料编码'},{key:'materialDescription',label:'物料描述',width:300},{key:'category',label:'材料类别'},
  ...
  {key:'currentStock',label:'现可用库存'},{key:'supplierUnreceived',label:'供应商未交量',width:120},{key:'plannedDemand',label:'计划需求量'},
  {key:'totalRequired',label:'应消耗量'},{key:'staticDate',label:'静态提出日期',width:112},{key:'lastYearInbound',label:'近一年入库量'},
...
const newStaticElectromechanicalColumns: Column[] = [
  {key:'material',label:'物料编码'},{key:'materialDescription',label:'物料描述',width:300},{key:'materialGroup',label:'物料组'},
  {key:'materialGroupName',label:'物料组描述',width:220},{key:'factorySet',label:'工厂编码'},{key:'factoryName',label:'工厂描述',width:180},
  {key:'unit',label:'单位'},{key:'totalRequired',label:'应消耗量'},{key:'staticDate',label:'静态提出日期',width:112},
...
const newStaticTabs: Record<NewStaticTab, NewStaticTabConfig> = {
  static: { label:'静态材料', total:137, filters:[newStaticBaseFilters[0],newStaticBaseFilters[1],{key:'development',label:'开发类型',options:DEVELOPMENT_OPTIONS},{key:'category',label:'材料类别',options:['板材','封边带','五金','电器']},newStaticBaseFilters[2],...maintenanceFilters('static')], columns:newStaticStaticColumns, rows:baseRows.filter(row=>row.category !== '电器'), templateFields:['工厂编码','工厂描述','物料编码','物料描述','处理方案类型','处理意见','方案制定部门','预计消耗完成时间','留用数量','留用时间','预计打样完成时间','静态原因'] },
  electromechanical: { label:'机电材料', total:28, filters:[...newStaticBaseFilters,...maintenanceFilters('electromechanical')], columns:newStaticElectromechanicalColumns, rows:baseRows.filter(row=>row.category === '电器'), templateFields:['工厂编码','工厂描述','物料编码','物料描述','处理方案类型','处理意见','方案制定部门','留用数量','留用时间','静态原因'] },
};
const electromechanicalConsumptionColumns: Column[] = [
  {key:'material',label:'物料编码'},{key:'materialDescription',label:'物料描述',width:300},{key:'factoryName',label:'工厂描述',width:180},
  {key:'development',label:'对应开发'},{key:'category',label:'材料类别'},{key:'unitName',label:'单位名称'},
  {key:'reportedStock',label:'提报时库存量',width:120},{key:'availableStock',label:'现可用库存',width:120},{key:'totalRequired',label:'应消耗总量'},
  ...
  {key:'retainedDate',label:'留用时间',width:112},{key:'totalOutbound',label:'总出库量'},{key:'outboundRatio',label:'出库比例'},
];
const staticConsumptionColumns: Column[] = [
  {key:'material',label:'物料编码'},{key:'materialDescription',label:'物料描述',width:300},{key:'factoryName',label:'工厂描述',width:180},
  {key:'development',label:'开发类型'},{key:'category',label:'材料类别'},{key:'unit',label:'单位'},
  {key:'reportedStock',label:'提报时库存量',width:120},{key:'availableStock',label:'现可用库存',width:120},{key:'supplierUnreceived',label:'供应商未交量',width:120},
  {key:'plannedDemand',label:'计划需求量'},{key:'totalRequired',label:'应消耗总量'},{key:'staticDate',label:'静态提出日期',width:112},
  {key:'solution',label:'处理方案类型',width:130},{key:'opinion',label:'处理意见',width:280},{key:'staticMonths',label:'静态月数'},
  {key:'receivedQuantity',label:'入库数量'},{key:'totalOutbound',label:'总出库量'},{key:'retention',label:'留存比例'},
  {key:'retainedQuantity',label:'留用数量'},{key:'retainedDate',label:'留用时间',width:112},{key:'expectedSampleDate',label:'预计打样完成时间',width:140},
...
const warningColumns: Column[] = [
  ...
  {key:'totalRequired',label:'应消耗总量'},{key:'currentStock',label:'现可用库存'},{key:'unitName',label:'单位'},
...
const reports: Record<StaticReportId, ReportConfig> = {
  ...
  electromechanical: { title:'机电材料跟踪消耗报表', total:84, filters:[{key:'staticDate',label:'静态提出日期',type:'month'},{key:'factoryName',label:'工厂描述',options:['定制公司供应工厂','板木公司供应工厂','清丰分公司供应工厂']},{key:'material',label:'物料编码',placeholder:'请输入物料编码'},{key:'development',label:'对应开发',options:['定制','板木','软体','配套']},{key:'category',label:'材料类别',options:['板材','封边带','五金','电器']},{key:'solution',label:'处理方案类型',options:configOptions('electromechanical', 'solution', true)},{key:'zeroStock',label:'现可用库存是否为0',options:['是','否']}], columns:electromechanicalConsumptionColumns, rows:baseRows.filter(row=>row.category === '电器') },
  'static-consumption': { title:'静态材料跟踪消耗报表', total:126, filters:[{key:'staticDate',label:'静态提出日期',type:'month'},{key:'factoryName',label:'工厂描述',options:['定制公司供应工厂','板木公司供应工厂','清丰分公司供应工厂']},{key:'development',label:'开发类型',options:['定制','板木','软体','配套']},{key:'category',label:'材料类别',options:['板材','封边带','五金','电器']},{key:'material',label:'物料编码',placeholder:'请输入物料编码'},{key:'solution',label:'处理方案类型',options:commonSolutions},{key:'zeroStock',label:'现可用库存是否为0',options:['是','否']}], columns:staticConsumptionColumns, rows:baseRows },
...
export default function StaticMaterialReports({ reportId }: { reportId: StaticReportId }) {
  ...
  const reportRows = useMemo(() => {
    ...
    return activeConfig.rows.map(row => {
      ...
      return { ...merged, expectedDate:usesSampleDate ? '' : merged.expectedDate, expectedSampleDate:usesSampleDate ? merged.expectedSampleDate : '', changeHistory: historyOverrides[key] ?? row.changeHistory ?? [] };
    });
  ...
  const rows = useMemo(() => reportRows.filter(row => activeConfig.filters.every(filter => {
    ...
    if (filter.key === 'zeroStock') { const stock=Number(row.availableStock ?? row.currentStock); return query === '是' ? stock === 0 : stock !== 0; }
