...
export default function StaticMaterialAgeConfig({ onBack }: Props) {
  ...
  const availableCategories = useMemo(() => devFilter ? Array.from(new Set(rules.filter((rule) => rule.dev === devFilter).map((rule) => rule.category).filter((item) => item !== '全部'))) : categoryOptions.filter((item) => item !== '全部'), [devFilter, rules]);
  ...
  return <div className="ioc-shell age-config-shell">
    ...
    <div className="workspace"><aside className="sidebar" onMouseLeave={() => setHoverMenu(null)}><div className="platform-title">IOC运营平台</div><nav>{menuItems.map(({ label, icon: Icon }, index) => <button type="button" key={label} className={`nav-item ${index === hoverMenu ? 'menu-active' : ''}`} onMouseEnter={() => setHoverMenu(index)} onClick={() => { if (index === 1) window.location.assign('/prototypes/order-track-report?view=static-material'); if (index === 2) window.location.assign('/prototypes/order-track-report'); }}><Icon size={17}/><span>{label}</span></button>)}</nav>{hoverMenu === 1 && <div className="ioc-sidebar-menu" style={{ top: 82 }}><h3>分析看板</h3><div className="ioc-sidebar-menu-list"><button type="button" className="selected" onClick={() => window.location.assign('/prototypes/order-track-report?view=static-material')}>静态材料跟踪处理分析</button></div></div>}{hoverMenu === 2 && <div className="ioc-sidebar-menu" style={{ top: 130 }}><h3>报表中心</h3><div className="ioc-sidebar-menu-list"><button type="button" onClick={() => window.location.assign('/prototypes/order-track-report')}>订单跟踪报表</button><button type="button" onClick={() => window.location.assign('/prototypes/order-track-report?report=custom-sales')}>定制接单打款销售统计报表</button></div></div>}{hoverMenu === 4 && <div className="ioc-sidebar-menu age-config-menu"><h3>配置中心</h3><div className="ioc-sidebar-menu-list"><button type="button" className="selected">静态材料库龄配置</button><button type="button" onClick={onBack}>自定义报表配置</button></div></div>}</aside>
