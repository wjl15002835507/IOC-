...
export default function StaticMaterialAgeConfig({ onBack }: Props) {
  ...
  return <div className="ioc-shell age-config-shell">
    ...
    <div className="workspace"><aside className="sidebar" onMouseLeave={() => setMenuOpen(false)}><div className="platform-title">IOC运营平台</div><nav>{menuItems.map(({ label, icon: Icon }, index) => <button type="button" key={label} className={`nav-item ${index === 4 && menuOpen ? 'menu-active' : ''}`} onMouseEnter={() => setMenuOpen(index === 4)} onClick={() => { if (index === 1) window.location.assign('/prototypes/order-track-report?view=static-material'); if (index === 2) window.location.assign('/prototypes/order-track-report'); }}><Icon size={17}/><span>{label}</span></button>)}</nav>{menuOpen && <div className="ioc-sidebar-menu age-config-menu"><h3>配置中心</h3><div className="ioc-sidebar-menu-list"><button type="button" className="selected">静态材料库龄配置</button><button type="button" onClick={onBack}>自定义报表配置</button></div></div>}</aside>
