  ...
  return <div className="ioc-shell age-config-shell">
    ...
    <div className="workspace"><aside className="sidebar" onMouseLeave={() => setMenuOpen(false)}><div className="platform-title">IOC运营平台</div><nav>{menuItems.map(({ label, icon: Icon }, index) => <button type="button" key={label} className={`nav-item ${index === 4 && menuOpen ? 'menu-active' : ''}`} onMouseEnter={() => setMenuOpen(index === 4)}><Icon size={17}/><span>{label}</span></button>)}</nav>{menuOpen && <div className="ioc-sidebar-menu age-config-menu"><h3>配置中心</h3><div className="ioc-sidebar-menu-list"><button type="button" className="selected">静态材料库龄配置</button><button type="button" onClick={onBack}>自定义报表配置</button></div></div>}</aside>
      <main className="main-area"><div className="tabbar"><span className="tab-close"><X size={14}/></span><span className="active-tab">静态材料库龄配置 <X size={12}/></span></div><div className="age-config-content">
        ...
        <section className="config-filter"><label>开发类型<select value={devFilter} onChange={(e) => setDevFilter(e.target.value)}><option value="">全部</option>{devOptions.map((item) => <option key={item}>{item}</option>)}</select></label><label>材料类别<select value={categoryFilter} onChange={(e) => setCategoryFilter(e.target.value)}><option value="">全部</option>{availableCategories.map((item) => <option key={item}>{item}</option>)}</select></label><label className="config-search">规则搜索<input value={query} onChange={(e) => setQuery(e.target.value)} placeholder="输入开发类型或材料类别"/></label><button type="button" className="text-button" onClick={() => { setDevFilter(''); setCategoryFilter(''); setQuery(''); }}>重置</button></section>
